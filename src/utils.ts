...
//     return answersList.words.includes(word) || answersList.valid.includes(word);
//   },
// };
...
export function checkHardMode(board: GameBoard, row: number): HardModeData {
  for (let i = 0; i < COLS; ++i) {
    if (
      board.state[row - 1][i] === "🟩" &&
      board.words[row - 1][i] !== board.words[row][i]
    ) {
  ...
  for (let i = 0; i < COLS; ++i) {
    if (
      board.state[row - 1][i] === "🟨" &&
      !board.words[row].includes(board.words[row - 1][i])
    ) {
      return { pos: i, char: board.words[row - 1][i], type: "🟨" };
    }
  }
...
export function getRowData(n: number, board: GameBoard) {
  const wordData = {
    ...
    letters: Array.from({ length: COLS }, () => ({
      val: null,
      not: new Set<string>(),
    })),
  };
...
export function getWordNumber(mode: GameMode) {
  return (
    Math.round(
      (modeData.modes[mode].seed - modeData.modes[mode].start) /
